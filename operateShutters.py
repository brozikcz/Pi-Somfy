...
class Shutter(MyLog):
    ...
    def sendCommand(self, shutterId, button, repetition):  # Sending a frame
        ...
        try:
            ...
            GPIO.setup(self.TXGPIO, GPIO.OUT)
            ...
            GPIO.output(self.TXGPIO, GPIO.HIGH)# wake up pulse
            usleep(9415)
            GPIO.output(self.TXGPIO, GPIO.LOW)# silence
            usleep(89565)

            for i in range(2):  # hardware synchronization
                GPIO.output(self.TXGPIO, GPIO.HIGH)
                usleep(2560)
                GPIO.output(self.TXGPIO, GPIO.LOW)
                usleep(2560)

            GPIO.output(self.TXGPIO, GPIO.HIGH) # software synchronization
            usleep(4550)
            GPIO.output(self.TXGPIO, GPIO.LOW)
            usleep(640)
            ...
            GPIO.output(self.TXGPIO, GPIO.LOW) # interframe gap
            usleep(30415)
